...
本项目不是普通的“AI 文案生成器”，而是一个包含多 Agent 工作流、Supervisor 调度、Prompt 模板管理、Agent Trace 可观测性、CSV 上传、上传字段校验、Markdown 报告导出与下载、Agent Trace JSON 导出与下载、evals 自动化评测的 Agent 应用工程项目。
...
12. Agent Trace 执行日志
13. Agent Trace JSON 导出与下载
14. 合规审核 Agent
15. Markdown 增长报告导出与下载
...
- 支持用户上传商品、销售和评论 CSV 数据，并进行字段完整性校验；构建 evals 自动化评测脚本，覆盖文件完整性、默认数据、examples 上传示例数据、运营指标生成、Prompt 渲染、报告导出函数和合规审核能力，当前静态评测 44/44 通过，带 LLM 评测 47/47 通过。
