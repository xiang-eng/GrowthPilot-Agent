...
本项目不是普通的“AI 文案生成器”，而是一个包含多 Agent 工作流、Supervisor 调度、Prompt 模板管理、Agent Trace 可观测性、CSV 模板下载、CSV 上传、上传字段校验、Markdown 报告导出与下载、Agent Trace JSON 导出与下载、evals 自动化评测的 Agent 应用工程项目。
...
13. Agent Trace 执行日志
14. Agent Trace JSON 导出与下载
15. 合规审核 Agent
16. Markdown 增长报告导出与下载
...
7. 点击“生成并准备下载 Agent Trace JSON”
8. 点击“下载 Agent Trace JSON”
9. 在本地或 `outputs/growth_report.md` 查看 Markdown 报告
10. 在本地或 `outputs/agent_trace.json` 查看结构化 Trace 日志
...
项目对 Agent 调用链路进行了工程化拆分，将数据处理、大模型调用、Prompt 模板、Agent 逻辑、报告导出和 evals 评测模块解耦；支持 Streamlit 侧边栏上传商品、销售和评论 CSV 数据，提供 CSV 模板下载，并对上传文件进行字段完整性校验，保证用户输入数据可用。

系统实现 Agent Trace 可观测性面板，记录每个 Agent 的执行状态、耗时、输入摘要、输出预览和错误信息；支持将 Supervisor 工作流结果导出并下载为 Markdown 增长报告，同时支持将 Agent Trace 导出为 JSON 文件，便于任务复盘、调试和后续平台化接入。

项目构建 evals 自动化评测脚本，覆盖文件完整性、默认数据字段、examples 上传示例数据、运营指标生成、Prompt 模板变量检查、Prompt 渲染、report_service 导出函数检查和合规审核能力，当前本地静态评测 44/44 通过，带 LLM 的合规审核评测 47/47 通过。
...
- 实现 Agent Trace 可观测性面板，记录每个 Agent 的输入摘要、执行状态、耗时、输出预览和错误信息，并支持将 Trace 日志导出为 JSON 文件，便于任务复盘和调试。
- 支持用户上传商品、销售和评论 CSV 数据，提供 CSV 模板下载和字段完整性校验；构建 evals 自动化评测脚本，覆盖文件完整性、默认数据、examples 上传示例数据、运营指标生成、Prompt 渲染、报告导出函数和合规审核能力，当前静态评测 44/44 通过，带 LLM 评测 47/47 通过。
...
6. 为什么要支持 Agent Trace JSON 导出
7. 如何用 Prompt 模板管理提升可维护性
8. 如何用 evals 检查项目稳定性
9. 如何通过合规审核 Agent 降低内容生成风险
10. 如何通过 Markdown 报告形成业务交付物
11. 如何通过 CSV 模板下载、上传和字段校验增强产品可用性
...
- `outputs/` 中的报告和 Trace JSON 是运行时生成结果，可以按需保留
- LLM 评测会消耗 API 调用额度
