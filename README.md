...
项目模拟小红书 / 抖音 / 内容电商平台中的商家运营流程，基于商品数据、销售数据和用户评论，结合内容平台规则、内容风格指南和合规风险词知识库，自动完成经营分析、用户洞察、批量增长策略生成、内容策略生成、合规审核、Agent Trace 记录和增长报告导出。

本项目不是普通的“AI 文案生成器”，而是一个包含多 Agent 工作流、Supervisor 调度、轻量 RAG 风格知识库增强、Prompt 模板管理、Agent Trace 可观测性、工作流 run_id 追踪、Supervisor 工作流进度展示、CSV 模板下载、CSV 上传、上传字段校验、Markdown 报告导出与下载、Agent Trace JSON 导出与下载、环境变量配置化、模型参数配置化、evals 自动化评测、`.gitignore` 安全上传控制和最终交付检查脚本的 Agent 应用工程项目。
...
10. 通过知识库沉淀平台规则、内容风格和合规风险词
11. 通过环境变量管理模型配置，提升部署和迁移便利性
12. 在项目交付前一键检查关键文件、语法和评测结果
13. 通过 `.gitignore` 防止密钥、缓存和运行产物被误传到 GitHub
...
- Prompt 模板文件管理
- Markdown 增长报告导出与下载
- knowledge_base 轻量知识库
- knowledge_service 知识库读取服务
- 内容策略 Agent 接入内容平台规则和内容风格知识库
- 合规审核 Agent 接入平台规则和合规风险词知识库
...
| Content Strategy Agent | 结合商品数据、评论数据和内容风格知识库，生成小红书标题、正文、抖音脚本和发布建议 |
| Compliance Agent | 结合待审核内容和合规知识库，审核内容中的合规风险 |
...
| 文件 | 作用 |
|---|---|
| platform_rules.md | 内容电商平台运营规则，包括真实性、功效宣传、绝对化用语、促销表达和用户评价引用规则 |
| content_style_guide.md | 小红书 / 抖音内容风格指南，包括标题风格、正文结构、短视频脚本结构和推荐表达方式 |
...
用于给 Compliance Agent 注入：

```text
平台规则 + 合规风险词
```

当前实现属于轻量 RAG 风格上下文注入，采用 Markdown 知识库读取和 Prompt 拼接方式。它还没有使用 Embedding、向量数据库和 Top-K 相似度检索，因此不是完整的 ChromaDB 向量检索 RAG。
...
- 方便后续版本迭代
- 更符合工程化开发习惯
- 便于 evals 检查 Prompt 变量完整性

---

### 3.7 CSV 上传、模板下载与字段校验
...
- `knowledge_base` 知识库文件是否存在
- `app/knowledge_service.py` 是否存在
- `knowledge_service` 核心函数是否存在
- 内容策略知识库是否能读取
- 合规知识库是否能读取
...
会额外真实调用 DashScope 通义千问模型，检查合规审核 Agent 是否能识别高风险营销内容。

如果 DashScope 欠费、额度不足、API Key 无权限或服务不可用，LLM 用例会自动标记为 SKIP，不会把外部服务状态误判为代码失败。
...
| Prompt 管理 | txt 模板文件 |
| 轻量知识库 | Markdown |
| 知识库读取服务 | app/knowledge_service.py |
...
docs/images/ 中的图片文件用于 GitHub README 展示。
knowledge_base/ 中的 Markdown 文件是轻量 RAG 风格知识库。
```
...
这种方式不是完整向量检索 RAG，但已经具备 RAG 风格的外部知识注入能力。
...
系统可以根据商品数据、销售数据和评论数据，结合轻量知识库，自动输出：
...
- 新增轻量 RAG 风格知识库，将内容平台规则、内容风格指南和合规风险词沉淀为 Markdown 文档，并通过 `knowledge_service.py` 分别注入 Content Strategy Agent 和 Compliance Agent 的 Prompt 中，提高内容生成的业务一致性和合规可控性。
- 新增多商品批量增长分析 Agent，支持用户同时选择多个商品，结合 GMV、CTR、CVR、退款率和用户评论进行商品优先级排序、分层运营策略生成和批量增长动作推荐。
- 对项目进行模块化工程拆分，将数据处理、大模型调用、Prompt 模板、Agent 逻辑、轻量知识库、环境变量配置、报告导出和 evals 评测解耦为独立模块，提升代码可维护性和扩展性。
...
- 支持用户上传商品、销售和评论 CSV 数据，提供 CSV 模板下载和字段完整性校验；构建 evals 自动化评测脚本，覆盖文件完整性、环境变量配置、轻量知识库、默认数据、examples 上传示例数据、运营指标生成、Prompt 渲染、Supervisor 静态能力、报告导出函数、Trace JSON 结构校验和合规审核能力，当前静态评测 96/96 通过。
- 新增 final_project_check.py 最终交付检查脚本，支持一键检查关键文件、环境变量文件、Python 语法、静态 evals 和带 LLM evals。
...
10. 如何用 Trace 记录每个 Agent 的执行过程
11. 为什么要记录 input_summary 输入摘要
12. 为什么要为每次工作流生成 run_id
13. 为什么要支持 Agent Trace JSON 导出
14. 为什么要给 Supervisor 工作流增加进度展示
15. progress_callback 是如何让前端感知 Agent 执行进度的
16. 为什么要把模型配置从代码中拆到 `.env`
17. 为什么 `call_qwen()` 要支持 `model` 和 `temperature` 参数
18. 为什么 evals 要检查 Trace JSON 文件结构
19. 为什么 evals 要检查 Supervisor 静态能力
20. 为什么 evals 要检查知识库文件和 knowledge_service
21. 为什么要增加 final_project_check.py 最终交付检查
22. 为什么要完善 `.gitignore` 防止密钥和运行产物被误传
23. 为什么 LLM 服务不可用时要标记 SKIP 而不是 FAIL
...
### 24.1 ChromaDB 向量检索 RAG

- 对 `knowledge_base/` 中的文档进行切分
- 使用 Embedding 模型生成向量
- 接入 ChromaDB 进行本地向量存储
- 根据商品、评论或待审核内容进行 Top-K 相似度检索
- 将检索结果注入 Agent Prompt
- 从轻量 RAG 升级为真正的向量检索 RAG
...
- 当前知识库增强属于轻量 RAG 风格上下文注入，还不是 ChromaDB 向量检索 RAG